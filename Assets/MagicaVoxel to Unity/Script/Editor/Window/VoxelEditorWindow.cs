namespace VoxeltoUnity {
	...
	public partial class VoxelEditorWindow : MoenenEditorWindow {
		...
		private const int EditorCount = 5;
		private static readonly string[] EDITOR_TITLE = new string[EditorCount] { "Rigging Editor", "Sprite Editor", "Map Generator", "Cha Generator", "Combiner" };
		private static readonly string[] HEADER_TITLE = new string[EditorCount] { "Rigging Editor", "Sprite Editor", "Map Generator", "Character Generator", "Prefab Combiner" };
		...
		[MenuItem("Tools/MagicaVoxel to Unity/Voxel Editor")]
		...
		private void HeaderGUI () {
			...
			LayoutH(() => {
				...
				// Save Data Button
				bool showSaveButton = !IsSpriting && !IsCombining;
		...
		private void PickGUI () {
			...
			{// Switch Button
				...
				rect.width += IsRigging ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				...
				rect.width = IsSpriting ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
				...
				rect.width = CurrentEditorMode == EditorMode.MapGenerator ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
				...
				rect.width = CurrentEditorMode == EditorMode.CharacterGenerator ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
				...
				rect.width = CurrentEditorMode == EditorMode.PrefabCombiner ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
		...
		private void DragInFileGUI () {
			if (!IsRigging && !IsSpriting && !IsCombining) { return; }
			if (Event.current.type == EventType.DragUpdated || Event.current.type == EventType.DragPerform) {
				if (DragAndDrop.paths.Length > 0) {
					var path = DragAndDrop.paths[0];
					if (!string.IsNullOrEmpty(path)) {
						var ex = Util.GetExtension(path);
						if ((IsRigging || IsSpriting) && (ex == ".vox" || ex == ".qb")) {
							// Voxel
							if (IsRigging && ex == ".qb") { return; }
						...
						} else if (IsCombining && Data && (ex == ".prefab" || ex == "")) {
							// Prefab
							if (Event.current.type == EventType.DragPerform) {
		...
		private void ViewGUI () {
			...
			if (CameraRoot && Camera && Camera.targetTexture) {
				...
				// Direction Sign
				bool showDirSign = ShowDirectionSign && (IsRigging || CurrentEditorMode == EditorMode.CharacterGenerator);
				...
				// Box
				bool showBox = ShowBackgroundBox && !IsCombining;
		...
		private void EditToolGUI () {
			...
			if (IsCombining || IsGenerating) { return; }
		...
		private void ToolbarGUI () {
			...
			LayoutH(() => {
				...
				if (IsRigging || CurrentEditorMode == EditorMode.CharacterGenerator) {
					// Direction Sign
			...
			switch (CurrentEditorMode) {
				...
				case EditorMode.Rigging:
					...
					if (RigAvailable) {
						...
						if (CombinerEditingRoot) { CombinerEditingRoot.gameObject.SetActive(false); }
				...
				case EditorMode.PrefabCombiner:
					...
					if (SpriteEditingRoot) { SpriteEditingRoot.gameObject.SetActive(false); }
					if (GeneratorEditingRoot) { GeneratorEditingRoot.gameObject.SetActive(false); }
					if (CombinerEditingRoot) { CombinerEditingRoot.gameObject.SetActive(true); }
					SetAllVoxelCollidersEnable(false);
