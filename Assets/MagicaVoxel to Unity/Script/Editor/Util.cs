namespace VoxeltoUnity {
	...
	public struct Util {
		...
		private const string ROOT_NAME = "MagicaVoxel to Unity";
		...
		public static string GetPrefabAssetPath (GameObject g) {
			...
			return g ? AssetDatabase.GetAssetPath(PrefabUtility.GetCorrespondingObjectFromSource(g)) : "";
#else
		...
		public static void OverrideMaterial (Material instance, Material data) {
			instance.shader = data.shader;
			instance.mainTexture = data.mainTexture;
			instance.CopyPropertiesFromMaterial(data);
		}



		public static void OverrideAvatar (Avatar instance, Avatar data) {
			JsonUtility.FromJsonOverwrite(JsonUtility.ToJson(data), instance);
		}
